...
timings / timingsGPU
@
This shows there is as much as an x-fold speed up by using the GPU for matrix multiplications! It's also interesting that the timing does not increase that much as r increases for the CPU version
